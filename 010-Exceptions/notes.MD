...
An exception is a Python object which represents an error. There are many specific built-in Exceptions that we have alreadty seen, such as `NameError` and `SyntaxError`. Specific exceptions should always be derived from the built-in `Exception` base class. In fact, exceptions are a fantastic example of inheritance, because they always inherit from `Exception`.
...
Recall that an `Exception` is just an object, meaning it can also have attributes. Specific information about an exception can be found within its attributes:
...
We can define our own exception as a class. It should, itself, inherit some type of exception.
